'use strict';
import {convertToEth, convertToGreg} from './converter'
import translate from '../utils/language'

const getHijraYear = (y) => {
  let ref_year = 2000
  let ref_hyear = 1428;

  let g = convertToGreg(1, 1, y);
  let  a = HijriJS.gregorianToHijri(g.year, g.month, g.day);
  return a.year
}
export const getIslamBeal = (y) => {
  let h_year = getHijraYear(y);
  let islamBeal = [];
  let ed_alfetir = {}
  let ed_aladha = {}
  let mawlid = {}
  ...
  let tmp = HijriJS.hijriToGregorian(h_year, 10, 1);
  tmp =   convertToEth(tmp.day, tmp.month, tmp.year);

  if(tmp.year < y){
    ed_alfetir = HijriJS.hijriToGregorian(h_year + 1, 10, 1);
    ed_aladha = HijriJS.hijriToGregorian(h_year+ 1, 12, 10);
    mawlid = HijriJS.hijriToGregorian(h_year+ 1, 3, 12);
    ramadan = HijriJS.hijriToGregorian(h_year+1, 9, 1);

    ed_alfetir =   convertToEth(ed_alfetir.day, ed_alfetir.month, ed_alfetir.year);
    ed_aladha =convertToEth(ed_aladha.day, ed_aladha.month, ed_aladha.year);
    mawlid =convertToEth(mawlid.day, mawlid.month, mawlid.year);
    ramadan =convertToEth(ramadan.day, ramadan.month, ramadan.year);
    ...
    ed_alfetir = HijriJS.hijriToGregorian(h_year, 10, 1);
    ed_aladha = HijriJS.hijriToGregorian(h_year, 12, 10);
    mawlid = HijriJS.hijriToGregorian(h_year, 3, 12);
    ramadan = HijriJS.hijriToGregorian(h_year, 9, 1);
  ...
  islamBeal.push({
    ...
    month: mawlid.month,
    days: mawlid.day,
    name: translate('HOLYDAY_mawlid')
  })
  islamBeal.push({
    year: ed_alfetir.year,
    month: ed_alfetir.month,
    days: ed_alfetir.day,
    name: translate('HOLYDAY_edalfetir')
  })

  islamBeal.push({
    year: ed_aladha.year,
    month: ed_aladha.month,
    days: ed_aladha.day,
    name: translate('HOLYDAY_edaladha')
  });
  islamBeal.push({
    year: ramadan.year,
    month: ramadan.month,
    days: ramadan.day,
    name: translate('HOLYDAY_ramadan')
  });

  return islamBeal;
}
/*
 * Thanks to ... for the contribution of this owesome hijira algorithm implementation
 *
 */
export var HijriJS = {
